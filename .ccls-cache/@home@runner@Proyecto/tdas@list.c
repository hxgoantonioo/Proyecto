...
void *list_search(List *list, void *key, int (*is_equal)(void*, void*)) {
...
void *list_popCurrent(List *L) {
  ...
  if (L->current == L->head) {
    ...
    if (L->head == NULL) {
      // Si eliminamos el único nodo, actualizar también el tail
      L->tail = NULL;
    }
  } else {
    // Recorrer la lista hasta el nodo anterior a current
    Node *temp = L->head;
    ...
    if (L->current == L->tail) {
      // Si eliminamos el último nodo, actualizar el tail
      L->tail = temp;
    }
  }

  // Liberar el nodo eliminado y actualizar el current
  Node *to_free = L->current;
  L->current = L->current->next;
  free(to_free);
  return data;
}
